...
- **`src/permissionAuthorizer.ts`** — optional, dependency-free integration with the separate [`@gotgenes/pi-permission-system`](https://pi.dev/packages/@gotgenes/pi-permission-system) extension (not in `package.json`; resolved dynamically via `import()` at runtime, and every step is wrapped in `try/catch` so absence or a signature mismatch no-ops silently). Registers an authorizer (`pi-small-models-sandbox`, via `registerSandboxAuthorizer(root)` from `session_start`, torn down on `session_shutdown`) that auto-`"allow"`s a path-shaped request only when this project's own sandbox would already allow it — reading `getSandboxState()` live, so while sandbox state is `"off"` it declines everything and defers to permission-system's own policy instead. Only takes effect once the user's own permission-system config also names `pi-small-models-sandbox` in its authorizer chain.
