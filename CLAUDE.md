...
- **`src/sandbox.ts`** — `resolveSandboxPath(root, target, mode)` is what every tool's `execute()` actually calls (`mode` is `"read"` for `find`/`grep`/`list`/`lstat`/`read`/`git_status`/`git_diff`, `"edit"` for `write`/`edit`/`insert`/`remove`/`mkdir`). It wraps `resolveSafePath` with a second, mode-aware layer: `READ_RESTRICTED_GLOBS` (SSH keys, `.aws`, `.env*`, `.netrc`, `.npmrc`, `.pgpass`, `.docker`, `.kube`, `.gnupg`, gcloud config, private-key filenames, and `.git/**`) blocks credential access in both modes — `.git/**` is included because `.git/config` routinely embeds credentials in remote URLs and `.git/objects`/`packed-refs` can retain secrets long deleted from the working tree, so it's a disclosure risk, not just a mutation one; `find`/`grep`/`list`'s default ignore globs keep `.git` out of an *unscoped* walk, but that's a convenience, not a security boundary, since it does nothing once a call points `path` straight at `.git`. `EDIT_RESTRICTED_GLOBS` is currently identical to `READ_RESTRICTED_GLOBS`. Globs are matched against both the lexical and real (symlink-resolved) path, so a symlink disguising itself as something else is still caught. A module-level `SandboxState` (`"on"`/`"off"`, default `"on"`) gates all of this: `"off"` disables root-containment *and* restricted globs — see `src/permissionGate.ts` for what fills that gap. Set via `/toggle-sandbox` (no argument toggles `on`/`off`). `isEntrySandboxSafe(base, entryPath, mode, isSymlink)` is the same check applied to entries a directory walk *discovers* (see below), and accepts an optional `stateOverride` for callers that don't share this module's in-memory state (namely `grepWorker.ts`, which runs in its own thread with its own copy starting at `"on"`).
