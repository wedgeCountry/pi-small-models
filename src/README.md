...
- `permissionAuthorizer.ts` — optional, dependency-free integration with the separate `@gotgenes/pi-permission-system` extension: while the sandbox is `"on"`, registers an authorizer that auto-allows a path-shaped request only when this sandbox's own containment-*and*-restricted-glob check would already allow it, so that extension doesn't ask about the same territory a second time; while `"off"`, declines everything so its own policy governs unmodified. No-ops if that package isn't installed.
