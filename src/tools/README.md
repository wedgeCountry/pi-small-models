...
`find`, `grep`, and `edit` share names with Pi's built-in tools, so registering them here replaces the
built-ins (per Pi's tool registry). `mkdir`, `remove`, `lstat`, and `insert` have no built-in name collision.
